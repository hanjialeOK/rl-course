...
def run_episode(model, env, discount_factor, render=False):
    ...
    discounted_future_rewards = discounted_future_rewards.to(torch.device("cuda:0"))
    loss = -torch.mean(action_chosen_log_probs * discounted_future_rewards)
...
def train(render=False):
    ...
    model = PolicyNetwork(input_size, hidden_size).to(torch.device("cuda:0"))
    ...
    while True:
        ...
        if batch % save_every_batches == 0:
            ...
            torch.save(save_dict, os.path.join('tensorboard_logs/'+start_time, 'checkpoint.pth'))
